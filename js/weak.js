...
const SciWeak = (() => {
  ...
  function recordAnswer(state, { termId, unit, correct, elapsedMs }) {
  ...
  function getWeakUnits(state, unitLabels) {
    ...
    for (const entry of log) {
      ...
      const s = (score.get(entry.unit) || 0) + 1 + (entry.guessed ? 0.5 : 0);
  ...
  function getWeakTerms(state, limit = 10) {
    ...
    for (const entry of log) {
      if (entry.correct) continue;
      const s = (score.get(entry.termId) || 0) + 1 + (entry.guessed ? 0.5 : 0);
  ...
  return { recordAnswer, getWeakUnits, getWeakTerms, FAST_GUESS_MS };
})();
