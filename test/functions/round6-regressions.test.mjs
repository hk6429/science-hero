...
function loadAppHarness() {
  ...
  const appSource = source('js/app.js').replace(
    ...
    `return { boot, __round6: {
      recordAnswer, answerFlash, renderFlashcard, renderQuiz, wireIoButtons, renderOnboarding,
      setState(value) { state = value; }, getState() { return state; },
      setFlashTerm(value) { flashQueue = [value]; flashIdx = 0; flashAnswering = false; },
      setLearningData(nextTerms) { terms = nextTerms; flashQueue = []; quizQueue = []; }
    } };`,
  );
  vm.runInContext(
...
test('A2：詞條已載入但目前沒有到期閃卡時顯示追平，不誤稱內容還在路上', () => {
  const harness = loadAppHarness();
  const term = { id: 'e-caught-up', unit: 'life', term: '蒸散作用' };
  harness.app.setLearningData([term]);
  harness.app.setState({
    cards: { [term.id]: { box: 2, due: Date.now() + 86400000, seen: 3, wrong: 0 } },
    stats: { streakDays: 0, lastActiveDate: null, totalReviews: 3 },
  });
  const body = harness.makeNode();

  harness.app.renderFlashcard(body);

  assert.match(body.innerHTML, /追平/);
  assert.doesNotMatch(body.innerHTML, /還在路上/);

  harness.app.setLearningData([]);
  harness.app.renderFlashcard(body);
  assert.match(body.innerHTML, /還在路上/, '詞條真的為空時仍保留載入提示');
});
