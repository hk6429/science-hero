...
function loadAppHarness() {
  ...
  return { app: context.__app, storage, appended, elements, answerEnergyCalls, makeNode };
}
...
test('A：box3 自測交替客觀選擇題與克漏字，客觀答對可升 box4 且 cloze 防火牆不退', () => {
  ...
  assert.deepEqual([0, 1, 2].map((index) => harness.app.selfTestUsesCloze(4, index)), [true, true, true], 'box4 維持克漏字回想');
