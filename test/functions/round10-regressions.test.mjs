...
test('C2：aside 慶祝吐司與科學驚喜都取得 celebrateIn 進場動畫', () => {
  const css = source('css/style.css');
  assert.match(css, /\.first-success\.celebrate-in\s*,\s*\.science-surprise\.celebrate-in\s*\{[^}]*animation:\s*celebrateIn\s+0\.35s\s+ease-out/s);
});
