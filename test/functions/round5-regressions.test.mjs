...
test('D：匯入既有進度但沒有本機旗標時，不重播 onboarding 與 first-success', () => {
  const { SciUiLogic } = loadClient(['js/ui-logic.js'], ['SciUiLogic']);
  const imported = { totalReviews: 12, masteredCount: 1, checklist: {}, firstSuccessSeen: false };
  assert.equal(SciUiLogic.shouldShowOnboarding(imported.totalReviews, imported.masteredCount, imported.checklist), false);
  assert.equal(SciUiLogic.shouldShowFirstSuccess(imported.totalReviews, imported.firstSuccessSeen), false);
  assert.equal(SciUiLogic.shouldShowOnboarding(0, 0, {}), true, '真正的新手仍要看到引導');
  assert.equal(SciUiLogic.shouldShowFirstSuccess(0, false), true, '真正的新手仍保留首次成功儀式');
